...
public class AnimalShelter {
	...
	private Queue<AnimalEntry> findQueueWithOldestAnimal(
			List<Queue<AnimalEntry>> queuesWithAnimals) {
		Queue<AnimalEntry> queueWithOldestAnimal = queuesWithAnimals.get(0);
		for (Queue<AnimalEntry> animalQueue : queuesWithAnimals) {
			if (animalQueue.peek().getInsertedEpochTime() > queueWithOldestAnimal
					.peek().getInsertedEpochTime()) {
	...
	private void verifyAnimalTypeSupported(AnimalType animalType) {
		if (!animals.containsKey(animalType)) {
			throw new NoSuchElementException("animal type not supported:"
					+ animalType);
		}
	}
